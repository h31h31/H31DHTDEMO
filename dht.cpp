...
static void search_step(struct search *sr, dht_callback *callback, void *closure)
{
    ...
    if(all_done) 
	{
		...
		else 
		{
            ...
            for(i = 0; i < sr->numnodes && j < 8; i++) 
			{
				...
				{
                    all_acked = 0;
                    _dout("Sending announce_peer.\n");
                    make_tid(tid, "ap", sr->tid);
                    send_announce_peer((struct sockaddr*)&n->ss,sizeof(struct sockaddr_storage),tid, 4, sr->id, sr->port, n->token, n->token_len,n->reply_time >= nowTime.tv_sec - 15);
                    n->pinged++;
                    n->request_time = nowTime.tv_sec;
                    node = find_node(n->id, n->ss.ss_family);
                    if(node) pinged(node, NULL);
        ...
        sr->step_time = nowTime.tv_sec;
        return;
    }

    if(sr->step_time + 15 >= nowTime.tv_sec)
        return;
    ...
    sr->done = 1;
    if(callback)
        (*callback)(closure,sr->af == AF_INET ? DHT_EVENT_SEARCH_DONE : DHT_EVENT_SEARCH_DONE6,sr->id, NULL, 0);
    sr->step_time = nowTime.tv_sec;
}
...
static struct search * new_search(void)
{
    ...
    if(oldest && oldest->step_time < nowTime.tv_sec - DHT_SEARCH_EXPIRE_TIME)
        return oldest;
...
int dht_search(const unsigned char *id, int port, int af,dht_callback *callback, void *closure)
{
    ...
    if(sr) {
        /* We're reusing data from an old search.  Reusing the same tid means that we can merge replies for both searches. */
        int i;
        sr->done = 0;
    again:
        for(i = 0; i < sr->numnodes; i++) {
            ...
            if(n->pinged >= 3 || n->reply_time < nowTime.tv_sec - 7200) {
...
static int storage_store(const unsigned char *id,const struct sockaddr *sa, unsigned short port)
{
    ...
    if(i < st->numpeers) {
        ...
        st->peers[i].time = nowTime.tv_sec;
        return 0;
    } 
	else 
	{
...
{
    struct storage *st = mystorage, *previous = NULL;
    while(st) {
        int i = 0;
        while(i < st->numpeers) {
            if(st->peers[i].time < nowTime.tv_sec - 32 * 60) {
                if(i != st->numpeers - 1)
        ...
        if(st->numpeers == 0) {
            ...
            numstorage--;
            if(numstorage < 0) {
                _dout("Eek... numstorage became negative.\n");
                numstorage = 0;
            }
        } else {
            previous = st;
...
static int rotate_secrets(void)
{
    ...
    rotate_secrets_time = nowTime.tv_sec + 900 + random() % 1800;
...
static void make_token(const struct sockaddr *sa, int old, unsigned char *token_return)
{
    ...
    dht_hash(token_return, TOKEN_SIZE,(char *)(old ? oldsecret : secret), sizeof(secret),(char *)ip, iplen, (char *)&port, 2);
}
...
static void dump_bucket(FILE *f, struct bucket *b)
{
    ...
    fprintf(f, " count %d age %d%s%s:\n",
            b->count, (int)(nowTime.tv_sec - b->time),
    ...
    while(n) {
        ...
        if(n->time != n->reply_time)
            fprintf(f, "age %ld, %ld",
                    (long)(nowTime.tv_sec - n->time),
                    (long)(nowTime.tv_sec - n->reply_time));
        else
            fprintf(f, "age %ld", (long)(nowTime.tv_sec - n->time));
        if(n->pinged)
...
void dht_dump_tables(FILE *f)
{
    ...
    while(sr) {
        ...
        fprintf(f, " age %d%s\n", (int)(nowTime.tv_sec - sr->step_time),
               sr->done ? " (done)" : "");
        for(i = 0; i < sr->numnodes; i++) {
            struct search_node *n = &sr->nodes[i];
            ...
            if(n->request_time)
                fprintf(f, "%d, ", (int)(nowTime.tv_sec - n->request_time));
            fprintf(f, "%d", (int)(nowTime.tv_sec - n->reply_time));
            if(n->pinged)
                fprintf(f, " (%d)", n->pinged);
            fprintf(f, "%s%s.\n",
    ...
    while(st) {
        ...
        for(i = 0; i < st->numpeers; i++) {
            ...
            fprintf(f, " %s:%u (%ld)",
                    buf, st->peers[i].port,
                    (long)(nowTime.tv_sec - st->peers[i].time));
        }
        st = st->next;
    }
...
int dht_init(int s, int s6, const unsigned char *id, const unsigned char *v)
{
    ...
    mybucket_grow_time = nowTime.tv_sec;
    mybucket6_grow_time = nowTime.tv_sec;
    confirm_nodes_time = nowTime.tv_sec + random() % 3;
    ...
    token_bucket_time = nowTime.tv_sec;
...
static int token_bucket(void)
{
    if(token_bucket_tokens == 0) {
        token_bucket_tokens = MIN(MAX_TOKEN_BUCKET_TOKENS,
                                  100 * (nowTime.tv_sec - token_bucket_time));
        token_bucket_time = nowTime.tv_sec;
    }
...
static int neighbourhood_maintenance(int af)
{
    ...
    if(q) {
		...
		{
            unsigned char tid[4];
            _dout("Sending find_node for%s neighborhood maintenance.\n",af == AF_INET6 ? " IPv6" : "");
            make_tid(tid, "fn", 0);
            send_find_node((struct sockaddr*)&n->ss, n->sslen,tid, 4, id, want,n->reply_time >= nowTime.tv_sec - 15);
            pinged(n, q);
        }
        return 1;
    }
                ...
                if(n) 
				{
                    ...
                    if(dht_socket >= 0 && dht_socket6 >= 0) 
					{
                        ...
                        if(otherbucket && otherbucket->count < 8)
                            ...
                            /* Most of the time, this just adds overhead.However, it might help stitch back one ofthe DHTs after a network collapse, so query both, but only very occasionally. */
                            want = WANT4 | WANT6;
                    }

                    _dout("Sending find_node for%s bucket maintenance.\n",af == AF_INET6 ? " IPv6" : "");
                    make_tid(tid, "fn", 0);
                    send_find_node((struct sockaddr*)&n->ss, n->sslen,tid, 4, id, want,n->reply_time >= nowTime.tv_sec - 15);
                    pinged(n, q);
...
int dht_periodic(const void *buf, size_t buflen,const struct sockaddr *fromAddr, int fromlen,time_t *tosleep,dht_callback *callback, void *closure)
{
    gettimeofday(&nowTime, NULL);
    ...
    if(buflen > 0) 
	{
        ...
        message = parse_message((unsigned char *)buf, buflen, tid, &tid_len, id, info_hash,
    ...
    //            } 
        ...
        case GET_PEERS:
			...
			else 
			{
				...
				if(callback) 
				{
					(*callback)(closure, DHT_EVENT_GETPEER_VALUES, info_hash,(void *)fromAddr, fromlen);
				}
...
/* We could use a proper bencoding printer and parser, but the format of  DHT messages is fairly stylised, so this seemed simpler. */
...
static int dht_send(const void *buf, size_t len, int flags,const struct sockaddr *sa, int salen)
{
    ...
    if(node_blacklisted(sa, salen)) {
        _dout("Attempting to send to blacklisted node.\n");
        errno = EPERM;
        return -1;
    }
	...
	struct sockaddr_in* tempip=(struct sockaddr_in *)sa;
	_dout("Send TO:[%s] port[%d] %s.\r\n",inet_ntoa(tempip->sin_addr),tempip->sin_port,buf);
	Sleep(100);
...
int send_find_node(const struct sockaddr *sa, int salen,const unsigned char *tid, int tid_len,const unsigned char *target, int want, int confirm)
{
    ...
    if(want > 0) {
        rc = _snprintf(buf + i, 512 - i, "4:wantl%s%se",(want & WANT4) ? "2:n4" : "",(want & WANT6) ? "2:n6" : "");
        INC(i, rc, 512);
    }
...
int send_nodes_peers(const struct sockaddr *sa, int salen,const unsigned char *tid, int tid_len,const unsigned char *nodes, int nodes_len,const unsigned char *nodes6, int nodes6_len,int af, struct storage *st,const unsigned char *token, int token_len)
{
    char buf[2048];
...
static int buffer_closest_nodes(unsigned char *nodes, int numnodes,const unsigned char *id, struct bucket *b)
{
    struct node *n = b->nodes;
    while(n) {
        if(node_good(n))
...
int send_closest_nodes(const struct sockaddr *sa, int salen,const unsigned char *tid, int tid_len,const unsigned char *id, int want,int af, struct storage *st,const unsigned char *token, int token_len)
{
    ...
    return send_nodes_peers(sa, salen, tid, tid_len,nodes, numnodes * 26,nodes6, numnodes6 * 38,af, st, token, token_len);
}
...
int send_announce_peer(const struct sockaddr *sa, int salen,unsigned char *tid, int tid_len,unsigned char *infohash, unsigned short port,unsigned char *token, int token_len, int confirm)
{
    char buf[512];
    int i = 0, rc;
	memset(buf,0,sizeof(buf));
...
static int send_error(const struct sockaddr *sa, int salen,unsigned char *tid, int tid_len,int code, const char *message)
{
    ...
    rc = _snprintf(buf + i, 512 - i, "d1:eli%de%d:",code, (int)strlen(message));
    INC(i, rc, 512);
...
static int parse_message(const unsigned char *buf, int buflen,unsigned char *tid_return, int *tid_len,unsigned char *id_return, unsigned char *info_hash_return,
              unsigned char *target_return, unsigned short *port_return,unsigned char *token_return, int *token_len,
              unsigned char *nodes_return, int *nodes_len,unsigned char *nodes6_return, int *nodes6_len,
              unsigned char *values_return, int *values_len,unsigned char *values6_return, int *values6_len,int *want_return)
{
    const unsigned char *p;

    /* This code will happily crash if the buffer is not NUL-terminated. */
    if(buf[buflen] != '\0') {
        _dout("Eek!  parse_message with unterminated buffer.\n");
        return -1;
    }
...
#define CHECK(ptr, len)                                                 \
    ...
    if(tid_return) {
        p = (unsigned char *)dht_memmem(buf, buflen,"1:t", 3);
        if(p) {
            long l;
            char *q;
    ...
    if(id_return) {
        p = (unsigned char *)dht_memmem(buf, buflen, "2:id20:", 7);
        if(p) {
            CHECK(p + 7, 20);
    ...
    if(info_hash_return) {
        p = (unsigned char *)dht_memmem(buf, buflen, "9:info_hash20:", 14);
        if(p) {
            CHECK(p + 14, 20);
    ...
    if(port_return) {
        p = (unsigned char *)dht_memmem(buf, buflen, "porti", 5);
        if(p) {
            long l;
            char *q;
    ...
    if(target_return) {
        p = (unsigned char *)dht_memmem(buf, buflen, "6:target20:", 11);
        if(p) {
            CHECK(p + 11, 20);
    ...
    if(token_return) {
        p = (unsigned char *)dht_memmem(buf, buflen, "5:token", 7);
        if(p) {
            long l;
            char *q;
    ...
    if(nodes_len) {
        p = (unsigned char *)dht_memmem(buf, buflen, "5:nodes", 7);
        if(p) {
            long l;
            char *q;
            l = strtol((char*)p + 7, &q, 10);
    ...
    if(nodes6_len) {
        p = (unsigned char *)dht_memmem(buf, buflen, "6:nodes6", 8);
        if(p) {
            long l;
            char *q;
    ...
    if(values_len || values6_len) {
        p = (unsigned char *)dht_memmem(buf, buflen, "6:valuesl", 9);
        if(p) {
            int i = p - buf + 9;
            int j = 0, j6 = 0;
            while(1) {
                ...
                if(q && *q == ':' && l > 0) {
                    ...
                    if(l == 6) {
                        ...
                        _dout("Received weird value -- %d bytes.\n", (int)l);
                    }
                } else {
                    break;
                }
            ...
            if(i >= buflen || buf[i] != 'e')
                _dout("eek... unexpected end for values.\n");
            if(values_len)
                *values_len = j;
            if(values6_len)
                *values6_len = j6;
    ...
    if(want_return) {
        p = (unsigned char *)dht_memmem(buf, buflen, "4:wantl", 7);
        if(p) {
            int i = p - buf + 7;
            *want_return = 0;
            while(buf[i] > '0' && buf[i] <= '9' && buf[i + 1] == ':' &&
                ...
                if(buf[i] == '2' && memcmp(buf + i + 2, "n4", 2) == 0)
                    ...
                    *want_return |= WANT6;
                    ...
                    _dout("eek... unexpected want flag (%c)\n", buf[i]);
                i += 2 + buf[i] - '0';
            }
            if(i >= buflen || buf[i] != 'e')
                _dout("eek... unexpected end for want.\n");
        } else {
            *want_return = -1;
        }
    }
...
#undef CHECK
    ...
    if((unsigned char *)dht_memmem(buf, buflen, "1:y1:r", 6))
        return REPLY;
    if((unsigned char *)dht_memmem(buf, buflen, "1:y1:e", 6))
        return ERROR;
